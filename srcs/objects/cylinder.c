...
static t_vector	get_cylinder_normal(t_cylinder *cyl, t_vector point,
			double proj)
{
	t_vector	normal;
	t_vector	axis_point;
	...
	else if (fabs(proj - cyl->height) <= EPSILON)
		normal = cyl->normalized;
	else
	{
		axis_point = vector_add(cyl->center,
				vector_scale(cyl->normalized, proj));
		normal = vector_normal(vector_sub(point, axis_point));
	}
	return (normal);
}
			...
			double t)
{
	...
	normal = get_cylinder_normal(cyl, pixel->point, projection);
